...
The bundled skills (`bish-arch`, `bish-audit-docs`, `bish-coverage`, `bish-grill-me`, `bish-work-on-card`) are vendored under `skills/` in the Bishop.AI repo, and `bishop install-skills` copies them to `%USERPROFILE%/.claude/skills/`. Skills version with Bishop; CLI evolution and skill text update in the same commit.
