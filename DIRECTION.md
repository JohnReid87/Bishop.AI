...
Spawns `wt -d <workspace_path>` (with `claude` launched in the shell). No WebView2 / xterm.js / ConPTY embedding. No card context passed via env vars — skills pull what they need from the `bishop` CLI.
...
- **Embedded terminal in Bishop.AI.** Cut — see above.
- **bishop.life.** Removed (card #1124/#1123) — the sister daily-reflection product (`life/` peer, `bish-life-*` skills) wasn't used and added ongoing repo complexity. Recoverable from git history if ever needed.
