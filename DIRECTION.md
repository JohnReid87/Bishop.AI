...
The bundled skills (`bish-arch`, `bish-audit-docs`, `bish-auto-card`, `bish-chat`, `bish-coverage`, `bish-grill-me`, `bish-onboard`, `bish-security`, `bish-tests`, `bish-triage`, `bish-work-on-card`, `bish-write-skill`) are vendored under `skills/` in the Bishop.AI repo, and `bishop install-skills` copies them to `%USERPROFILE%/.claude/skills/`. Skills version with Bishop; CLI evolution and skill text update in the same commit.
