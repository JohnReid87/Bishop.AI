...
The UI is the primary surface for kanban work (cards, lanes, tags); the `bishop` CLI is the automation surface for skills like `work-on-card-bishop`.
...
- After MSI install: run `bishop install-skills` once to populate `~/.claude/skills/` with the bundled Claude Code skills (`work-on-card-bishop`, `grill-me-bishop`).
