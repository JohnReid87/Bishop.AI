...
namespace Bishop.Tests.Cli.Batches.CleanUp;

[Collection("ConsoleTests")]
public sealed class CleanUpBatchCliCommandTests
{
    private static Workspace MakeWorkspace() =>
        new() { Id = Guid.NewGuid(), Name = "test-ws", Path = @"C:\repos\MyProject" };

    [Fact]
    public async Task InvokeAsync_HappyPath_ExitsZeroAndSendsCleanUpCommand()
    {
        ...
        await mediator.Received(1).Send(
            Arg.Is<CleanUpBatchCommand>(c => c.Name == "Sprint 1" && c.WorkspacePath == ws.Path),
            Arg.Any<CancellationToken>());
        output.ToString().Should().Contain("Batch cleaned up.");
    }
