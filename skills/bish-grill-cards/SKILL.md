...
bishop.command: /bish-grill-cards {{card_number}}
bishop.stage: true
...
- Strip a leading `#` if present, then call:
  ```
  bishop context-pack grill-cards --card <number>
  ```
...
```
bishop context-pack grill-cards
```
...
`model: "haiku"`). Brief it with the specific question and ask it to return
`file:line` excerpts — never pull whole files into this conversation.
