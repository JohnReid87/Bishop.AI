...
2. If `bish-onboard` itself is installed but any other `bish-*` skill expected by the current Bishop release is missing, run `bishop install-skills`.
   - You can determine which skills the current Bishop release ships by globbing for `skills/bish-*` next to `bishop.exe`, or simply by checking that `bish-arch`, `bish-audit-docs`, `bish-coverage`, `bish-grill-cards`, `bish-tests`, `bish-work-on-card`, and `bish-onboard` are all present.
...
- Run `/bish-grill-cards` to plan your next chunk of work and push cards to the board.
```
