...
## Batch accent palette — group identity

Six neon slots cycle across batch group header left-borders, letting
the user distinguish groups at a glance. The palette is intentionally
louder than the tag family — a 3 px structural border strip does not
break the "no large fills" rule for signal green.

| Slot | Hex       | Reads as        |
| ---- | --------- | --------------- |
| 0    | `#00FF41` | signal green    |
| 1    | `#BF40FF` | electric violet |
| 2    | `#1A8CFF` | electric blue   |
| 3    | `#FF1493` | hot pink        |
| 4    | `#00FFFF` | neon cyan       |
| 5    | `#FF6B00` | neon orange     |

Slot assigned sequentially by `WorkspaceBoardViewModel`: open batches are
sorted by `CreatedAt` ascending (workspace-scoped), and each receives its
list position as its `AccentIndex`. The same batch shows the same accent
...
threshold before they ship — see `HexColorToContrastForegroundConverter` in the UI layer.
...
Bishop.AI ships dark-mode only — light-mode variants are not shipped and
not maintained.
