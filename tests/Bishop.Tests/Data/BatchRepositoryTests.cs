...
public sealed class BatchRepositoryTests : IClassFixture<DbFixture>
{
    ...
    public async Task GetByBranchName_ReturnsMatchingBatch()
    {
        ...
        var repo = Repo();
        var branchName = U("br");
        await repo.CreateAsync(U("name"), branchName, "main", @"C:\worktree");
    ...
    public async Task AssignCard_Throws_WhenCardAlreadyInNonClosedBatch()
    {
        // Arrange
        var repo = Repo();
        var batch1 = await repo.CreateAsync(U("name"), U("branch"), "main", @"C:\worktree");
        var batch2 = await repo.CreateAsync(U("name"), U("branch"), "main", @"C:\worktree");
        var cardId = await CreateCardAsync();
    ...
    public async Task List_ReturnsBatchesOrderedByCreatedAt()
    {
        ...
        var branch1 = U("br");
        var branch2 = U("br");
        var b1 = await repo.CreateAsync(U("name"), branch1, "main", @"C:\worktree");
        await Task.Delay(10); // ensure distinct CreatedAt
        var b2 = await repo.CreateAsync(U("name"), branch2, "main", @"C:\worktree");
