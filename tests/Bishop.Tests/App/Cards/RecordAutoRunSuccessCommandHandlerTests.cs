...
public sealed class RecordAutoRunSuccessCommandHandlerTests : IClassFixture<DbFixture>
{
    private readonly BishopDbContext _db;
    private readonly IDbContextFactory<BishopDbContext> _factory;
    private readonly SqliteConnection _connection;

    public RecordAutoRunSuccessCommandHandlerTests(DbFixture fixture)
    {
        _db = fixture.Db;
    ...
    [Fact]
    public async Task Handle_SetsLastAutoRunSucceededAt()
    {
        // Arrange
        var cardId = await CreateCardAsync();
        var sut = new RecordAutoRunSuccessCommandHandler(_factory, TimeProvider.System);

        // Act
        await sut.Handle(new RecordAutoRunSuccessCommand(cardId), default);

        // Assert
        var saved = await _db.Cards.AsNoTracking().SingleAsync(c => c.Id == cardId);
        saved.LastAutoRunSucceededAt.Should().NotBeNull();
    }
    ...
    public async Task Handle_Throws_WhenCardDoesNotExist()
    {
        ...
        // Act
        var act = () => sut.Handle(new RecordAutoRunSuccessCommand(Guid.NewGuid()), default);
