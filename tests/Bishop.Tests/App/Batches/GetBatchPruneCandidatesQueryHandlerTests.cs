...
public sealed class GetBatchPruneCandidatesQueryHandlerTests : IClassFixture<DbFixture>
{
    ...
    public GetBatchPruneCandidatesQueryHandlerTests(DbFixture fixture) => _factory = fixture.Factory;
        ...
        BatchClosedReason reason, DateTimeOffset? closedAt = null)
    {
        ...
        var batch = await repo.CreateAsync(U("batch"), $"bishop/{U("br")}", "main", @"C:\wt");
    ...
    private GetBatchPruneCandidatesQueryHandler CreateHandler(IGitCli git)
        => new(new BatchRepository(_factory), git);
    ...
    public async Task NoCandidates_WhenNoBatchesExist()
    {
        ...
        var result = await CreateHandler(git).Handle(
            new GetBatchPruneCandidatesQuery(WorkspacePath, false, false, null), default);
    ...
    {
        var repo = new BatchRepository(_factory);
        var open = await repo.CreateAsync(U("batch"), $"bishop/{U("br")}", "main", @"C:\wt");
    ...
    public async Task AbandonedOnlyFilter_ExcludesFinished()
    {
        ...
        var result = await CreateHandler(git).Handle(
            new GetBatchPruneCandidatesQuery(WorkspacePath, AbandonedOnly: true, MergedOnly: false, OlderThan: null), default);
    ...
    public async Task MergedOnlyFilter_ExcludesAbandoned()
    {
        ...
        var result = await CreateHandler(git).Handle(
            new GetBatchPruneCandidatesQuery(WorkspacePath, AbandonedOnly: false, MergedOnly: true, OlderThan: null), default);
    ...
    public async Task OlderThanFilter_ExcludesTooRecent()
    {
        ...
        var result = await CreateHandler(git).Handle(
            new GetBatchPruneCandidatesQuery(WorkspacePath, false, false, TimeSpan.FromDays(7)), default);
