...
public sealed class PrintContextQueryHandlerTests : IDisposable
{
    ...
    [Fact]
    public void StripLabel_NoParentheses_ReturnsInputUnchanged()
    {
        var result = PrintContextQueryHandler.StripLabel("Shell selection");

        result.Should().Be("Shell selection");
    }

    [Fact]
    public void StripLabel_OpenParenAtPositionZero_ReturnsInputUnchanged()
    {
        // lastParen == 0 fails the lastParen > 0 guard, so the text is returned as-is
        var result = PrintContextQueryHandler.StripLabel("(Bar)");

        result.Should().Be("(Bar)");
    }

    [Fact]
    public void StripLabel_NormalLabel_StripsLabelAndTrims()
    {
        var result = PrintContextQueryHandler.StripLabel("Shell selection (STABLE)");

        result.Should().Be("Shell selection");
    }

    [Fact]
    public void StripLabel_SpacesBeforeLabel_AreTrimmedFromResult()
    {
        var result = PrintContextQueryHandler.StripLabel("Foo Bar  (Label)");

        result.Should().Be("Foo Bar");
    }

    [Fact]
    public async Task Handle_NullWorkspacePath_ThrowsArgumentNullException()
    {
        var act = () => _handler.Handle(new PrintContextQuery(null!), default);

        await act.Should().ThrowAsync<ArgumentNullException>();
    }

    [Fact]
    public async Task Handle_EmptyWorkspacePath_ThrowsContextFileNotFound()
    {
        var act = () => _handler.Handle(new PrintContextQuery(""), default);
    ...
    [Fact]
    public async Task Handle_EmptySectionName_ThrowsUnknownSection()
    {
        var act = () => _handler.Handle(new PrintContextQuery(_tempDir, ""), default);
    ...
    [Fact]
    public void ParseH2Sections_EmptyContent_ReturnsEmptyList()
    {
        var sections = PrintContextQueryHandler.ParseH2Sections("");

        sections.Should().BeEmpty();
    }

    [Fact]
    public void ParseH2Sections_NoH2Headers_ReturnsEmptyList()
    {
        const string content = "# Title\n\nSome text without any H2 headers.\n";

        var sections = PrintContextQueryHandler.ParseH2Sections(content);

        sections.Should().BeEmpty();
    }

    [Fact]
    public void ParseH2Sections_WhitespaceOnlyHeaderText_YieldsSectionWithEmptyName()
    {
        const string content = "## \n\nSome body text.\n";

        var sections = PrintContextQueryHandler.ParseH2Sections(content);

        sections.Should().ContainSingle().Which.Name.Should().BeEmpty();
    }
}
