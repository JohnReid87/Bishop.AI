...
- `skills/` — vendored Claude Code skill files (`bish-arch`, `bish-audit-docs`, `bish-auto-card`, `bish-chat`, `bish-coverage`, `bish-grill-me`, `bish-onboard`, `bish-security`, `bish-tests`, `bish-triage`, `bish-work-on-card`, `bish-write-skill`) shipped with `bishop.exe` and installed to `~/.claude/skills/` via `bishop install-skills`.
