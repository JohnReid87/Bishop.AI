...
public sealed class StreamJsonFormatter
{
    ...
    private void EmitStatus(string label)
    {
        ...
        var suffix = RunFormatting.FormatTokenSuffix(RunningInputTokens, RunningOutputTokens, RunningCacheCreationTokens + RunningCacheReadTokens);
    ...
    private string? FormatResult(JsonElement root)
    {
        ...
        var tokenSegment = RunFormatting.FormatFinalTokenSegment(Totals);
        if (tokenSegment is not null)
            summary += $" · {tokenSegment}";
    ...
    private static bool TryReadModelUsageTotals(JsonElement root, out ClaudeRunTotals? totals)
    {
        ...
        foreach (var model in modelUsage.EnumerateObject())
        {
            ...
            inputTokens += ReadInt(model.Value, "inputTokens");
            outputTokens += ReadInt(model.Value, "outputTokens");
            cacheCreation += ReadInt(model.Value, "cacheCreationInputTokens");
            cacheRead += ReadInt(model.Value, "cacheReadInputTokens");
            hasAny = true;
        }
