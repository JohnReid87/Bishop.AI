...
public sealed class ReconcileOrphanedBatchesCommandHandler : IRequestHandler<ReconcileOrphanedBatchesCommand>
{
    ...
    public ReconcileOrphanedBatchesCommandHandler(IBatchRepository batches, IDbContextFactory<BishopDbContext> dbFactory)
    {
        _batches = batches;
    ...
    {
        var allBatches = await _batches.ListAsync(cancellationToken);
