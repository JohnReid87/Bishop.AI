...
public sealed partial class WorkspaceDetailPage : Page
{
    ...
    private void Card_RightTapped(object sender, RightTappedRoutedEventArgs e)
    {
        ...
        var flyout = new CommandBarFlyout { Placement = FlyoutPlacementMode.Bottom };

        var openBtn = new AppBarButton
        {
            Icon = new SymbolIcon(Symbol.OpenFile),
            Label = "Open",
        };
        ToolTipService.SetToolTip(openBtn, "Open detail");
        openBtn.Click += (_, _) => _ = _safeAsync.RunAsync(async () =>
        {
            var vm = await _dialogService.ShowCardDetailDialogAsync(card, Board.CardSkills, _item?.Path ?? string.Empty, _item?.Id ?? Guid.Empty, _item?.GitHubRepo, XamlRoot);
            if (vm.Deleted || vm.Updated)
                await Board.RefreshCommand.ExecuteAsync(null);
        });
        flyout.PrimaryCommands.Add(openBtn);

        var closeBtn = new AppBarButton
        {
            Icon = new FontIcon { Glyph = card.CloseReopenGlyph },
            Label = card.IsClosed ? "Reopen" : "Close",
        };
        ToolTipService.SetToolTip(closeBtn, card.CloseReopenTooltip);
        closeBtn.Click += (_, _) => _ = _safeAsync.RunAsync(async () =>
        {
            await Board.ToggleCardClosedAsync(card.Id, card.IsClosed);
            card.IsClosed = !card.IsClosed;
            Board.RefreshLaneItems();
        });
        flyout.PrimaryCommands.Add(closeBtn);

        var moveBtn = new AppBarButton
        {
            Icon = new SymbolIcon(Symbol.MoveToFolder),
            Label = "Move",
        };
        ToolTipService.SetToolTip(moveBtn, "Move to lane");
        var moveSub = new MenuFlyout();
        foreach (var lane in SystemLaneNames.All)
        {
            var laneName = lane;
            var laneItem = new MenuFlyoutItem
            {
                Text = laneName,
                IsEnabled = !string.Equals(card.LaneName, laneName, StringComparison.Ordinal),
            };
            laneItem.Click += (_, _) => _ = _safeAsync.RunAsync(async () =>
            {
                await Board.MoveCardAsync(card.Id, laneName, 0);
                await Board.RefreshCommand.ExecuteAsync(null);
            });
            moveSub.Items.Add(laneItem);
        }
        moveBtn.Flyout = moveSub;
        flyout.PrimaryCommands.Add(moveBtn);
        ...
        deleteBtn.Click += (_, _) => _ = _safeAsync.RunAsync(async () =>
        {
            var confirmed = await ConfirmFlyoutAsync(anchor, $"Delete card #{card.Number}? This cannot be undone.", "Delete");
            if (!confirmed) return;
            await Board.DeleteCardAsync(card.Id);
            await Board.RefreshCommand.ExecuteAsync(null);
        });
