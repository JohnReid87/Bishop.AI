...
public sealed class BatchRepository : IBatchRepository
{
    ...
    public async Task<Batch> CreateAsync(Guid workspaceId, string name, string branchName, string baseBranch, string worktreePath, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IReadOnlyList<Batch>> ListAsync(Guid workspaceId, CancellationToken cancellationToken = default)
    {
        await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
        var batches = await db.Batches.Where(b => b.WorkspaceId == workspaceId).ToListAsync(cancellationToken);
    ...
    public async Task<Batch> SetStoppedAtAsync(Guid batchId, DateTimeOffset? value, CancellationToken cancellationToken = default)
    {
        ...
        var batch = await db.Batches.FirstOrDefaultAsync(b => b.Id == batchId, cancellationToken)
            ?? throw new InvalidOperationException($"Batch {batchId} not found.");
        batch.StoppedAt = value;
        await db.SaveChangesAsync(cancellationToken);
        return batch;
    }

    public async Task<Batch> SetFinishedAtAsync(Guid batchId, DateTimeOffset? value, CancellationToken cancellationToken = default)
    {
        await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
        var batch = await db.Batches.FirstOrDefaultAsync(b => b.Id == batchId, cancellationToken)
            ?? throw new InvalidOperationException($"Batch {batchId} not found.");
    ...
    public async Task<Batch> RenameAsync(Guid batchId, string newName, CancellationToken cancellationToken = default)
    {
        await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
        var batch = await db.Batches.FirstOrDefaultAsync(b => b.Id == batchId, cancellationToken)
            ?? throw new InvalidOperationException($"Batch {batchId} not found.");
